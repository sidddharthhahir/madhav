"""Hindi/Gujarati translation CLI. Same shape as gita.enrich.run, deliberately.

    python -m gita.translate.run --dry-run --limit 2   # render a prompt, cost only
    python -m gita.translate.run --submit               # create the batch
    python -m gita.translate.run --status                # poll
    python -m gita.translate.run --collect                # write results
...
def _latest_batch(conn):
    return conn.execute(
        """SELECT batch_id, status, submitted_at, model FROM translate_batches
            ORDER BY submitted_at DESC LIMIT 1"""
    ).fetchone()


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Build Hindi/Gujarati translations.")
    ...
    if not pending:
        print("every verse already has both Hindi and Gujarati; nothing to do")
        return 0
    ...
    print("pending     : %d verses (both languages per request)" % len(pending))
    ...
    if args.dry_run:
        ...
        for name, spec in P.TRANSLATION_SCHEMA["properties"].items():
            print("  %-10s %-7s %s" % (name, spec["type"], spec["description"][:70] + "..."))
        return 0
    ...
    print("poll with:   python -m gita.translate.run --status")
    print("then:        python -m gita.translate.run --collect")
    return 0
